...
void ErrorHandler::report(CompilerError &error)
{
    ...
    std::cerr << COLOR_BOLD << COLOR_RED << "[" << displayName << "] " << COLOR_RESET << baseName << error.line << ":" << error.col << " " << error.message << "\n";
...
std::string ErrorHandler::getBaseName(const std::string &fullPath)
{
    if (fullPath.empty())
    {
        std::cerr << "[ERROR HANDLER WARNING] fullPath is empty!\n";
        return "unknown_file";
    }
...
void HintGenerator::registerHintFns()
{
    ...
    hintGeneratorFnsMap[ErrorLevel::SEMANTIC] = &HintGenerator::generateSemanticHints;
}

std::vector<std::string> HintGenerator::generateHints(const CompilerError &error, std::string sourceLine)
{
...
std::vector<std::string> HintGenerator::generateParserHints(const CompilerError &error, std::string sourceLine)
{
    std::cout << "PARSER HINT GENERATION TRIGGERED\n";
...
std::vector<std::string> HintGenerator::generateSemanticHints(const CompilerError &error, std::string sourceLine)
{
    std::cout << "SEMANTIC HINT GENERATION TRIGGERED\n";
    ...
    static const std::vector<HintRule> rules = {
        ...
        // Type mismatch ( expected X but got Y)
        {
            ...
            {
                return std::vector<std::string>{
                    "Convert '" + m[2].str() + "' to '" + m[1].str() + "' or adjust the variable type"};
            }},
        ...
        // Cannot convert implicitly(This will come in handy when I add casting)
        {
            ...
            {
                return std::vector<std::string>{
                    "Consider explicit casting or using the correct type: '" + m[1].str() + "' → '" + m[2].str() + "'"};
            }}};
