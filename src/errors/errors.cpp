...
ErrorHandler::ErrorHandler(const std::string &file) : fileName(file) { loadSourceLines(); }
...
void ErrorHandler::report(CompilerError &error) {
    ...
    // Header
    std::cerr << COLOR_BOLD << COLOR_RED << "[" << displayName << "] " << COLOR_RESET << baseName
    ...
    for (int i = startLine; i <= endLine; i++) {
        ...
        if (i == error.line) {
            ...
            for (int j = 0; j < error.col - 1 && j < static_cast<int>(line.length()); j++) {
                if (line[j] == '\t') {
                    std::cerr << "\t";
...
int ErrorHandler::getTokenLength(Node *contextNode) {
    ...
    if (auto fnDeclExpr = dynamic_cast<FunctionDeclarationExpression *>(contextNode))
