...
void Auditor::classifyNode(Node *node) {
    ...
    logInternal("[CLASSIFY-NODE] Entering with node type: " + std::string(typeid(*node).name()) +
...
void Auditor::classifyBlock(Node *block) {
    ...
    if (auto blockStmt = dynamic_cast<BlockStatement *>(block)) {
        ...
        logInternal(
            "[CLASSIFY-BLOCK] ERROR: Block is neither BlockStatement nor "
            "BlockExpression!");
        return;
    }
    ...
    for (Node *stmt : statements) {
        logInternal("[CLASSIFY-BLOCK] Processing statement " + std::to_string(stmtIndex++));
        ...
        if (auto declaration = dynamic_cast<VariableDeclaration *>(actualNode)) {
            ...
            for (const auto &ident : idents) {
                classifySymbol(ident, block, info.get());
            }
        }
    }

    logInternal("[CLASSIFY-BLOCK] Finished. Natives: " + std::to_string(info->natives.size()) +
...
void Auditor::classifySymbol(Node *node, Node *block, BlockInfo *info) {
    ...
    if (!sym || !sym->storage().isHeap) {
        logInternal("[CLASSIFY-SYM] No heap symbol for node, skipping");
        return;
    }
...
void Auditor::classifyClause(SwitchStatement *sw, Node *stmt,
    ...
    logInternal("[CLAUSE-CLASSIFIER] Switch being classified: " + (sw ? sw->toString() : "NULL"));
    logInternal("[CLAUSE-CLASSIFIER] Clause vector size: " + std::to_string(clause.size()));
    ...
    logInternal("  - Original node type: " + (stmt ? std::string(typeid(*stmt).name()) : "NULL"));
    logInternal("  - Peeled node type: " + (actual ? std::string(typeid(*actual).name()) : "NULL"));
    logInternal("  - Are they the same? " + std::string((stmt == actual) ? "YES" : "NO"));

    // If the statement is a block call the normal block classifier
    if (isBlock(actual)) {
        logInternal("[CLAUSE-CLASSIFIER] Node is a block - delegating to classifyNode");
        logInternal("  - Block type detected, recursing...");
        classifyNode(actual);
        logInternal("[CLAUSE-CLASSIFIER] Block classification complete");
    } else {
        logInternal("[CLAUSE-CLASSIFIER] Node is not a block - treating as floater");
        ...
        logInternal("  - Is pointer: " + std::string(sym->type().isPointer ? "YES" : "NO"));
        ...
        logInternal("[CLAUSE-CLASSIFIER] Checking if symbol was born in this clause");
        ...
        if (bornHere) {
            ...
            logInternal("[CLAUSE-CLASSIFIER] NATIVE DETECTED: " + sym->codegen().ID);
            logInternal("  - Adding to natives list for switch: " + sw->toString());
            logInternal("  - Natives count now: " + std::to_string(blockInfo->natives.size()));
        } else {
            logInternal("[CLAUSE-CLASSIFIER] Not born here - checking if it dies in switch");
            logInternal("  - Calling doesSymbolDieInSwitch for ID: " + sym->codegen().ID);

            bool diesHere = doesSymbolDieInSwitch(sym->codegen().ID, clause);
            logInternal("  - Dies in switch? " + std::string(diesHere ? "YES" : "NO"));

            if (diesHere) {
                logInternal("[CLAUSE-CLASSIFIER] FOREIGNER DETECTED: " + sym->codegen().ID);
                logInternal("  - Adding to foreigners list for switch: " + sw->toString());
                blockInfo->foreigners.push_back(sym->codegen().ID);
                logInternal("  - Foreigners count now: " +
                            std::to_string(blockInfo->foreigners.size()));
            } else {
                logInternal(
                    "[CLAUSE-CLASSIFIER] Symbol neither native nor foreigner "
                    "in this switch");
    ...
    logInternal("  - Current natives in switch: " + std::to_string(blockInfo->natives.size()));
...
bool Auditor::shouldForeignBunkerBlock(Node *block) {
    ...
    if (it == deferedFrees.end() || !it->second) return false;
...
bool Auditor::shouldNativeBunkerBlock(Node *block) {
    logInternal("[SHOULD-BUNKER] Checking block at: " + std::to_string((uintptr_t)block));
    ...
    logInternal("[SHOULD-BUNKER] BlockInfo has " + std::to_string(it->second->natives.size()) +
                " natives");

    // A block needs bunkering if it has any natives
    bool result = !it->second->natives.empty();
    logInternal("[SHOULD-BUNKER] Returning: " + std::string(result ? "true" : "false"));
    return result;
}
...
void Auditor::bunkerNatives(Node *block) {
    ...
    for (const auto &nativeID : blockInfo->natives) {
        if (isBunkered(nativeID)) {
            logInternal("[NATIVE BUNKERING] Baton " + nativeID + " already bunkered, skipping");
            continue;
        }
        ...
        if (!holder) {
            ...
            logInternal(
                "[NATIVE BUNKERING] Could not find the baton holder for "
                "lifetime ID:" +
                nativeID + " skipping...");
            continue;
        }
        ...
        if (!holderSym) {
            logInternal("[NATIVE BUNKERING] Failed to get the symbol info for holder " +
                        holder->toString() + " for lifetime ID: " + nativeID + " skipping...");
            continue;
        }
        ...
        if (!baton) {
            scanForBaton(nativeID);
            logInternal(
                "[NATIVE BUNKERING] Could not find the baton for "
                "lifetime ID:" +
                nativeID + " skipping...");
            continue;
        }
...
void Auditor::bunkerForeigners(Node *block) {
    logInternal("[FOREIGNER BUNKERING] Starting action for block: " + block->toString());

    auto &blockInfo = deferedFrees[block];
    if (!blockInfo) {
        logInternal(
            "[FOREIGNER BUNKERING] No BlockInfo (To-Do list) found for "
            "this block address. Bailing.");
        return;
    }

    logInternal("[FOREIGNER BUNKERING] Found " + std::to_string(blockInfo->foreigners.size()) +
    ...
    for (const auto &foreignerID : blockInfo->foreigners) {
        ...
        if (!holder) {
            ...
            logInternal(
                "[FOREIGNER BUNKERING] FAILED: Could not find the baton "
                "holder for ID: " +
                foreignerID);
            continue;
        }
        ...
        if (!baton) {
            scanForBaton(foreignerID);
            logInternal("[FOREIGNER BUNKERING] FAILED: No baton in responsibilityTable for " +
                        foreignerID);
            continue;
        }

        // SUCCESS LOGS
        logInternal("[FOREIGNER BUNKERING] SUCCESS: Moving baton for " + foreignerID +
                    " into bunker.");

        foreignersToFree[block].push_back({std::move(baton), holderSym});
        bunkeredIDs.insert(foreignerID);

        logInternal("[FOREIGNER BUNKERING] ID " + foreignerID + " is now officially bunkered.");
    }
}
...
void Auditor::bunkerCycles(Node *block) {
    ...
    logInternal("[BUNKER-CYCLES] BlockInfo has " + std::to_string(blockInfo->natives.size()) +
                " natives");
    logInternal("[BUNKER-CYCLES] BlockInfo has " + std::to_string(blockInfo->foreigners.size()) +
                " foreigners");
    ...
    logInternal("[BUNKER-CYCLES] cycleGroup.ids.size() = " + std::to_string(cycleGroup.ids.size()));
    ...
    for (const auto &validID : cycleGroup.ids) {
        ...
        if (isBunkered(validID)) {
            logInternal("[CYCLE BUNKERING] Baton " + validID + " already bunkered, skipping");
            continue;
        }
        ...
        if (!holder) {
            ...
            logInternal(
                "[CYCLE BUNKERING] Could not find the baton holder for "
                "lifetime ID:" +
                validID + " skipping...");
            continue;
        }
        ...
        auto holderSym = semantics.getSymbolFromMeta(holder);
        if (!holderSym) {
            logInternal("[BUNKER-CYCLES] holderSym is NULL for holder: " + holder->toString());
            logInternal("[CYCLE BUNKERING] Failed to get the symbol info for holder " +
        ...
        if (!baton) {
            ...
            scanForBaton(validID);
            logInternal(
                "[CYCLE BUNKERING] Could not find the baton for "
                "lifetime ID:" +
                validID + " skipping...");
            continue;
        }
        ...
        // SUCCESS LOGS
        logInternal("[CYCLE BUNKERING] SUCCESS: Moving baton for " + validID + " into bunker.");

        nativesToFree[block].push_back({std::move(baton), holderSym});
        bunkeredIDs.insert(validID);
        bunkeredCount++;

        logInternal("[CYCLE BUNKERING] ID " + validID + " is now officially bunkered.");
    }

    logInternal("[BUNKER-CYCLES] Total bunkered: " + std::to_string(bunkeredCount));
...
void Auditor::bunkerPersists(Node *block) {
    ...
    for (const auto &persistID : blockInfo->natives) {
        ...
        if (!sym) reportDevBug("Failed to get holder symbol info", holder);
...
bool Auditor::isBlock(Node *node) {
    ...
    if (dynamic_cast<ifStatement *>(node)) return true;
    if (dynamic_cast<BlockStatement *>(node)) return true;
...
bool Auditor::checkBirth(const std::string &id,
                         const std::vector<std::unique_ptr<Statement>> &clause) {
    ...
    for (const auto &stmt : clause) {
        ...
        if (isDeclaration(actual)) {
            ...
            if (!declSym) {
                logInternal(
                    "[CHEKCING BIRTH] Failed to find declaration symbol info "
                    "skipping...");
                continue;
            }
...
std::vector<std::string> Auditor::getAllActiveBatonIDsInBlock(Node *block) {
    logInternal(
        "[SELECTED_SCAN] Identifying all live batons in "
    ...
    for (const auto &[node, baton] : semantics.responsibilityTable) {
        ...
        logInternal("[SELECTED_SCAN] Inspecting Table Entry: Node(" + node->toString() +
                    ") -> BatonID(" + baton->ID + ")");

        if (baton->isResponsible) {
            Node *holder = semantics.queryForLifeTimeBaton(baton->ID);
            if (!holder) {
                logInternal("[SELECTED_SCAN] ! ERROR: No holder found for ID: " + baton->ID);
                continue;
            }
            ...
            auto holderSym = semantics.getSymbolFromMeta(holder);
            if (!holderSym) {
                logInternal("[SELECTED_SCAN] ! ERROR: No symbol metadata for holder of: " +
                            baton->ID);
                continue;
            }

            logInternal("[SELECTED_SCAN] Checking Stats for " + baton->ID + ": isResponsible(" +
...
std::set<std::string> Auditor::getAllActiveBatonIDs() {
    logInternal("[ACTIVE_SCAN] Identifying all live batons in responsibilityTable...");
    ...
    for (const auto &[node, baton] : semantics.responsibilityTable) {
        ...
        logInternal("[ACTIVE_SCAN] Inspecting Table Entry: Node(" + node->toString() +
                    ") -> BatonID(" + baton->ID + ")");

        if (baton->isResponsible) {
            Node *holder = semantics.queryForLifeTimeBaton(baton->ID);
            if (!holder) {
                logInternal("[ACTIVE_SCAN] ! ERROR: No holder found for ID: " + baton->ID);
                continue;
            }

            auto holderSym = semantics.getSymbolFromMeta(holder);
            if (!holderSym) {
                logInternal("[ACTIVE_SCAN] ! ERROR: No symbol metadata for holder of: " +
                            baton->ID);
                continue;
            }

            logInternal("[ACTIVE_SCAN] Checking Stats for " + baton->ID + ": isResponsible(" +
                        std::to_string(baton->isResponsible) + ") refCount(" +
                        std::to_string(holderSym->storage().refCount) + ") ptrCount(" +
                        std::to_string(holderSym->storage().pointerCount) + ")");
...
bool Auditor::containsNode(Node *root, Node *target) {
    ...
    if (auto *addr = dynamic_cast<AddressExpression *>(root)) {
        if (containsNode(addr->identifier.get(), target)) return true;
    }

    if (auto *deref = dynamic_cast<DereferenceExpression *>(root)) {
        if (containsNode(deref->identifier.get(), target)) return true;
    }

    if (auto *arrSub = dynamic_cast<ArraySubscript *>(root)) {
        if (containsNode(arrSub->identifier.get(), target)) return true;

        for (const auto &len : arrSub->index_exprs) {
            if (containsNode(len.get(), target)) return true;
        }
    }

    if (auto *self = dynamic_cast<SelfExpression *>(root)) {
        for (auto &field : self->fields) {
            if (containsNode(field.get(), target)) return true;
        }
    }

    if (auto *newComp = dynamic_cast<NewComponentExpression *>(root)) {
        for (auto &arg : newComp->arguments) {
            if (containsNode(arg.get(), target)) return true;
        }
    }

    if (auto *call = dynamic_cast<CallExpression *>(root)) {
        if (containsNode(call->function_identifier.get(), target)) return true;
        for (auto &param : call->parameters) {
            if (containsNode(param.get(), target)) return true;
        }
    }

    if (auto *unwrap = dynamic_cast<UnwrapExpression *>(root)) {
        if (containsNode(unwrap->expr.get(), target)) return true;
    }

    if (auto *method = dynamic_cast<MethodCallExpression *>(root)) {
        if (containsNode(method->instance.get(), target)) return true;
        if (containsNode(method->call.get(), target)) return true;
    }

    if (auto *instance = dynamic_cast<InstanceExpression *>(root)) {
        if (instance->blockIdent) {
            if (containsNode(instance->blockIdent.get(), target)) return true;
        }
    ...
    if (auto *funcExpr = dynamic_cast<FunctionExpression *>(root)) {
        ...
        if (funcExpr->return_type) {
            if (containsNode(funcExpr->return_type.get(), target)) return true;
        }
        if (funcExpr->block) {
            if (containsNode(funcExpr->block.get(), target)) return true;
        }
    }

    if (auto *cast = dynamic_cast<CastExpression *>(root)) {
        if (cast->expr && containsNode(cast->expr.get(), target)) return true;
    }

    if (auto *bitcast = dynamic_cast<BitcastExpression *>(root)) {
        if (bitcast->expr && containsNode(bitcast->expr.get(), target)) return true;
    }

    if (auto *prefix = dynamic_cast<PrefixExpression *>(root)) {
        if (containsNode(prefix->operand.get(), target)) return true;
    }

    if (auto *postfix = dynamic_cast<PostfixExpression *>(root)) {
        if (containsNode(postfix->operand.get(), target)) return true;
    }

    if (auto *infix = dynamic_cast<InfixExpression *>(root)) {
        if (containsNode(infix->left_operand.get(), target)) return true;
        if (containsNode(infix->right_operand.get(), target)) return true;
    }

    if (auto *arrayLit = dynamic_cast<ArrayLiteral *>(root)) {
        for (auto &item : arrayLit->array) {
            if (containsNode(item.get(), target)) return true;
        }
    }
    ...
    if (auto *fStr = dynamic_cast<FStringLiteral *>(root)) {
        for (const auto &seg : fStr->segments) {
            for (const auto &val : seg.values) {
                if (containsNode(val.get(), target)) return true;
            }
        }
    }

    if (auto *subscript = dynamic_cast<ArraySubscript *>(root)) {
        if (subscript->identifier && containsNode(subscript->identifier.get(), target)) return true;
        for (auto &index : subscript->index_exprs) {
            if (containsNode(index.get(), target)) return true;
        }
    }

    // STATEMENTS
    if (auto *exprStmt = dynamic_cast<ExpressionStatement *>(root)) {
        if (containsNode(exprStmt->expression.get(), target)) return true;
    }

    if (auto *decl = dynamic_cast<VariableDeclaration *>(root)) {
        if (decl->initializer && containsNode(decl->initializer.get(), target)) return true;
    }

    if (auto *assign = dynamic_cast<AssignmentStatement *>(root)) {
        if (containsNode(assign->identifier.get(), target)) return true;
        if (containsNode(assign->value.get(), target)) return true;
    }

    if (auto *fieldAssign = dynamic_cast<FieldAssignment *>(root)) {
        if (containsNode(fieldAssign->lhs_chain.get(), target)) return true;
        if (containsNode(fieldAssign->value.get(), target)) return true;
    }

    if (auto *ret = dynamic_cast<ReturnStatement *>(root)) {
        if (ret->return_value && containsNode(ret->return_value.get(), target)) return true;
    }
    ...
    if (auto *ifStmt = dynamic_cast<ifStatement *>(root)) {
        if (ifStmt->condition && containsNode(ifStmt->condition.get(), target)) return true;
        if (ifStmt->if_result && containsNode(ifStmt->if_result.get(), target)) return true;
        for (const auto &elif : ifStmt->elifClauses) {
            if (containsNode(elif.get(), target)) return true;
        }
        if (ifStmt->else_result.has_value() && ifStmt->else_result.value()) {
            if (containsNode(ifStmt->else_result.value().get(), target)) return true;
        }
    }

    if (auto *elif = dynamic_cast<elifStatement *>(root)) {
        if (elif->elif_condition && containsNode(elif->elif_condition.get(), target)) return true;
        if (elif->elif_result && containsNode(elif->elif_result.get(), target)) return true;
    }

    if (auto *switchStmt = dynamic_cast<SwitchStatement *>(root)) {
        if (switchStmt->switch_init && containsNode(switchStmt->switch_init.get(), target))
            return true;
        for (auto &caseClause : switchStmt->case_clauses) {
            if (containsNode(caseClause.get(), target)) return true;
        }
        for (auto &stmt : switchStmt->default_statements) {
            if (containsNode(stmt.get(), target)) return true;
        }
    }

    if (auto *caseClause = dynamic_cast<CaseClause *>(root)) {
        if (caseClause->condition && containsNode(caseClause->condition.get(), target)) return true;
    ...
    if (auto *whileStmt = dynamic_cast<WhileStatement *>(root)) {
        if (whileStmt->condition && containsNode(whileStmt->condition.get(), target)) return true;
        if (whileStmt->loop && containsNode(whileStmt->loop.get(), target)) return true;
    }

    if (auto *forStmt = dynamic_cast<ForStatement *>(root)) {
        if (forStmt->initializer && containsNode(forStmt->initializer.get(), target)) return true;
        if (forStmt->condition && containsNode(forStmt->condition.get(), target)) return true;
        if (forStmt->step && containsNode(forStmt->step.get(), target)) return true;
        if (forStmt->body && containsNode(forStmt->body.get(), target)) return true;
    }

    if (auto *blockStmt = dynamic_cast<BlockStatement *>(root)) {
        for (auto &stmt : blockStmt->statements) {
            Node *peeled = peelNode(stmt.get());
            if (containsNode(peeled, target)) return true;
        }
    }
    ...
    if (auto *blockExpr = dynamic_cast<BlockExpression *>(root)) {
        ...
        if (blockExpr->finalexpr.has_value() && blockExpr->finalexpr.value()) {
            if (containsNode(blockExpr->finalexpr.value().get(), target)) return true;
        }
    }

    if (auto *funcStmt = dynamic_cast<FunctionStatement *>(root)) {
        if (funcStmt->funcExpr && containsNode(funcStmt->funcExpr.get(), target)) return true;
    }

    if (auto *funcDecl = dynamic_cast<FunctionDeclaration *>(root)) {
        if (funcDecl->function_name && containsNode(funcDecl->function_name.get(), target))
            return true;
        for (auto &param : funcDecl->parameters) {
            if (containsNode(param.get(), target)) return true;
        }
        if (funcDecl->return_type && containsNode(funcDecl->return_type.get(), target)) return true;
    }

    if (auto *funcDeclExpr = dynamic_cast<FunctionDeclarationExpression *>(root)) {
        if (funcDeclExpr->funcDeclrStmt && containsNode(funcDeclExpr->funcDeclrStmt.get(), target))
            return true;
    }

    if (auto *comp = dynamic_cast<ComponentStatement *>(root)) {
        for (auto &method : comp->methods) {
            if (containsNode(method.get(), target)) return true;
        }

        if (comp->initConstructor.has_value() && comp->initConstructor.value()) {
            if (containsNode(comp->initConstructor.value().get(), target)) return true;
        }
    }
    ...
    if (auto *init = dynamic_cast<InitStatement *>(root)) {
        ...
        if (init->block && containsNode(init->block.get(), target)) return true;
    }

    if (auto *seal = dynamic_cast<SealStatement *>(root)) {
        if (seal->block && containsNode(seal->block.get(), target)) return true;
    }

    if (auto *generic = dynamic_cast<GenericStatement *>(root)) {
        if (generic->block && containsNode(generic->block.get(), target)) return true;
    }

    if (auto *alloc = dynamic_cast<AllocatorStatement *>(root)) {
        if (alloc->block && containsNode(alloc->block.get(), target)) return true;
    }
...
void Auditor::simulateDeclFree(VariableDeclaration *declaration, const std::string &contextID) {
    ...
    if (!holderNode) {
        logInternal("    [DECL-SIMULATE FREE] No holder node found for " + contextID);
        return;
    }

    // Check if the holder node is the actual contextNode
    if (holderNode != declaration) {
        logInternal("[DECL-SIMULATE FREE] The baton was moved node: " + declaration->toString() +
                    "' is not the executor");
        return;
    }
...
void Auditor::simulateFree(Node *contextNode, const std::string &contextID) {
    ...
    for (const auto &identifier : identifiers) {
        ...
        if (holderNode != identifier) {
            logInternal("[SIMULATE FREE] The baton was moved node: " + identifier->toString() +
                        "' is not the executor");
            return;
        }
...
void Auditor::assignmentRob(Node *contextAssign, const std::shared_ptr<SymbolInfo> &assignSym) {
    ...
    } else if (auto fieldAssign = dynamic_cast<FieldAssignment *>(contextAssign)) {
        valNode = fieldAssign->value.get();
        lhsNode = fieldAssign->lhs_chain.get();
    }

    if (!valNode || !lhsNode) reportDevBug("Failed to get nodes for assignment", contextAssign);
    ...
    for (const auto &identifier : identifiers) {
        ...
        if (valSym->storage().isHeap) {
            ...
            if (valBatonHolder == identifier) {
                logInternal("  [ASSIGNMENT HEIST] ValBaton " + valBaton->ID +
                            " isResponsible: " + (valBaton->isResponsible ? "YES" : "NO"));

                if (assignSym->type().isPointer) {
                    valBaton->ptrCount++;
                    logInternal("[ASSIGNMENT HEIST] Incrementing pointerCount for " + valBaton->ID +
                                " to " + std::to_string(valBaton->ptrCount));
                }

                if (valBaton->isResponsible) {
                    logInternal("  [ASSIGNMENT HEIST] Attempting Robbery: " + robberBaton->ID +
                                " robbing " + valBaton->ID);
                    semantics.transferResponsibility(robberBaton.get(), valBaton.get(), valSym);
                } else {
                ...
                // Force a robbery
                logInternal(
                    "  [ASSIGNMENT HEIST] valBatonHolder is not current value "
    ...
    if (!info) {
        logInternal(
            "[ASSIGNMENT HEIST] No BlockInfo for current block, skipping "
            "cycle check");
        return;
    }
...
void Auditor::transferDependent(const std::string &dependentID,
    ...
    if (regIt == candidateRegistry.end()) {
        logInternal("      [STATUS] Victim " + dependentID + " not in Candidate Registry.");
        if (dropCount) {
            dependentSym->storage().pointerCount--;
            logInternal("      [ACTION] dropCount=TRUE. PointerCount for " + dependentID +
    ...
    for (const auto &robberID : robbers) {
        ...
        if (!robberBatonHolder) {
            logInternal("        [SKIP] Robber " + robberID + " has no baton holder node.");
            continue;
        }

        auto &robberBaton = semantics.responsibilityTable[robberBatonHolder];
        logInternal("        [STATE] isAlive: " + std::string(robberBaton->isAlive ? "YES" : "NO") +
                    " | isResponsible: " + std::string(robberBaton->isResponsible ? "YES" : "NO"));
        ...
        if (robberBaton->isAlive) {
            ...
            if (robberBaton->isResponsible) {
                ...
                if (masterBatonHolder) {
                    ...
                    logInternal("        [SUCCESS] Loot " + dependentID + " moved to Master " +
                                masterID + " briefcase.");
                } else {
    ...
    if (!adoptionPassed) {
        logInternal("      [FINAL-JUDGMENT] No living heirs found for " + dependentID);
        if (dropCount) {
            dependentSym->storage().pointerCount--;
            logInternal("      [ACTION] dropCount=TRUE. Final PointerCount for " + dependentID +
...
bool Auditor::diesInBlock(const std::string &ID, Node *block) {
    logInternal("[DIES-IN-BLOCK] Checking if ID: " + ID + " dies in block: " + block->toString());
...
bool Auditor::doesSymbolDieInSwitch(const std::string &id,
                                    const std::vector<std::unique_ptr<Statement>> &clause) {
    ...
    for (const auto &stmt : clause) {
        ...
        bool contains = std::find_if(clause.begin(), clause.end(), [holder](const auto &ptr) {
                            return ptr.get() == holder;
                        }) != clause.end();

        logInternal("[DIES-IN-CLAUSE] Holder for " + id + " is " + holder->toString());
        logInternal("[DIES-IN-CLAUSE] Does clause contain holder? " +
                    std::string(contains ? "YES" : "NO"));

        if (!contains) {
            logInternal("[DIES-IN-CLAUSE] Bailing because holder is outside this clause");
            continue;
        }
        ...
        logInternal(
            "[DIES-IN-CLAUSE] PtrCount: " + std::to_string(holderSym->storage().pointerCount) +
            " | RefCount: " + std::to_string(holderSym->storage().refCount) +
            " | Responsible: " + (baton->isResponsible ? "T" : "F"));
...
void Auditor::scanForBaton(const std::string &id) {
    ...
    Node *holder = semantics.queryForLifeTimeBaton(id);
    if (!holder) {
        logInternal("[SCANNER] LAYER 1:  Baton " + id + " NOT FOUND in responsibility table");
    } else {
        logInternal("[SCANNER] LAYER 1: Found baton " + id + " in responsibility table");
        logInternal("  Holder node: " + holder->toString());

        auto &baton = semantics.responsibilityTable[holder];
        if (baton) {
            logInternal("  Baton state:");
            logInternal("    - isResponsible: " + std::string(baton->isResponsible ? "YES" : "NO"));
            logInternal("    - isAlive: " + std::string(baton->isAlive ? "YES" : "NO"));
            logInternal("    - ownedBy: " + baton->ownedBy);
            logInternal("    - Dependents count: " + std::to_string(baton->dependents.size()));
        } else {
    ...
    for (const auto &[block, batonList] : nativesToFree) {
        for (size_t i = 0; i < batonList.size(); ++i) {
            ...
            if (baton && baton->ID == id) {
                foundInNatives = true;
                logInternal("[SCANNER] LAYER 2: Found baton " + id + " in nativesToFree");
    ...
    if (!foundInNatives) {
        logInternal("[SCANNER] LAYER 2: Baton " + id + " NOT FOUND in nativesToFree");
    }
    ...
    for (const auto &[block, batonList] : foreignersToFree) {
        for (size_t i = 0; i < batonList.size(); ++i) {
            ...
            if (baton && baton->ID == id) {
                ...
                logInternal("[SCANNER] LAYER 3: Found baton " + id + " in foreignersToFree");
                logInternal("  Block: " + block->toString());
                logInternal("  Position in list: " + std::to_string(i));
                logInternal("  Symbol info: " + (sym ? sym->codegen().ID : "null"));
                logInternal("  Baton state:");
    ...
    if (!foundInForeigners) {
        logInternal("[SCANNER] LAYER 3: Baton " + id + " NOT FOUND in foreignersToFree");
    }

    logInternal("\n--- SCAN SUMMARY ---");
    logInternal("Baton ID: " + id);
    logInternal("Layer 1 (Responsibility Table): " + std::string(holder ? "FOUND" : "NOT FOUND"));
    ...
    if (!holder && !foundInNatives && !foundInForeigners) {
        ...
        logInternal("[SCANNER]  WARNING: Baton " + id + " appears in MULTIPLE layers!");
    }
}
...
bool Auditor::isAlreadyClassified(const std::string &id, BlockInfo *blockInfo) {
    ...
    if (std::find(blockInfo->natives.begin(), blockInfo->natives.end(), id) !=
        ...
        logInternal("[CLASSIFIER CHECK] ID: " + id + " is already native classified");
        return true;
    }

    // Check foreigners list
    if (std::find(blockInfo->foreigners.begin(), blockInfo->foreigners.end(), id) !=
        blockInfo->foreigners.end()) {
        logInternal("[CLASSIFIER CHECK] ID: " + id + " is already foreign classified");
        return true;
    }
...
bool Auditor::isCycle(LifeTime *baton) {
    ...
    logInternal("[IS-CYCLE] baton.isResponsible = " +
                std::string(baton->isResponsible ? "true" : "false"));
    logInternal("[IS-CYCLE] baton.dependents.size() = " + std::to_string(baton->dependents.size()));
...
bool Auditor::followDependents(LifeTime *current, const std::string &originalID,
    ...
    logInternal("[FOLLOW] Current: " + current->ID +
                " (dependents: " + std::to_string(current->dependents.size()) + ")");
    ...
    logInternal("[FOLLOW] Added to visited and cycle. visited size: " +
                std::to_string(visited.size()) + ", cycle size: " + std::to_string(cycle.size()));

    // Follow dependents (what this baton points to)
    int depIndex = 0;
    for (const auto &[depID, depSym] : current->dependents) {
        logInternal("[FOLLOW] Checking dependent " + std::to_string(depIndex++) + ": " + depID);
    ...
    logInternal("[FOLLOW] After backtrack - cycle size: " + std::to_string(cycle.size()) +
                ", visited size: " + std::to_string(visited.size()));
    return false;
}

const CycleGroup Auditor::findCycleGroup(const std::unique_ptr<BlockInfo> &blockInfo) {
    CycleGroup group;
    ...
    logInternal("[FIND-CYCLE] BlockInfo has " + std::to_string(blockInfo->natives.size()) +
                " natives");
    logInternal("[FIND-CYCLE] BlockInfo has " + std::to_string(blockInfo->foreigners.size()) +
                " foreigners");

    std::set<std::string> processed;

    std::vector<std::string> allBatons;
    allBatons.insert(allBatons.end(), blockInfo->natives.begin(), blockInfo->natives.end());
    allBatons.insert(allBatons.end(), blockInfo->foreigners.begin(), blockInfo->foreigners.end());

    logInternal("[FIND-CYCLE] Total batons to check: " + std::to_string(allBatons.size()));

    int batonIndex = 0;
    for (const auto &id : allBatons) {
        logInternal("\n[FIND-CYCLE] Checking baton " + std::to_string(batonIndex++) +
                    ": ID = " + id);
        ...
        logInternal("[FIND-CYCLE] isCycle returned: " + std::string(hasCycle ? "true" : "false"));
        ...
        if (hasCycle) {
            ...
            for (const auto &cycleID : cycle) {
                ...
                // Check if native
                if (std::find(blockInfo->natives.begin(), blockInfo->natives.end(), cycleID) !=
                    blockInfo->natives.end()) {
            ...
            cycle.clear();
            logInternal("[FIND-CYCLE] Cycle processed, cleared temporary cycle vector");
        } else {
    ...
    logInternal("[FIND-CYCLE] group.ids.size() = " + std::to_string(group.ids.size()));
...
void Auditor::checkCycleSafety(const std::unique_ptr<BlockInfo> &blockInfo, Node *contextNode) {
    auto cycleGroup = findCycleGroup(blockInfo);
    logInternal("[CYCLE CHECK] Checking cycle safety");

    std::string nativeList = getIDsString(cycleGroup.ids, blockInfo->natives);
    std::string foreignerList = getIDsString(cycleGroup.ids, blockInfo->foreigners);
    ...
    if (cycleGroup.hasNative && cycleGroup.hasForeigner) {
        ...
        logAuditError("Illegal cycle detected between native and foreigner lifetimes", contextNode);
    }
}
