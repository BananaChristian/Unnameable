...
llvm::Value *IRGenerator::generateIdentifierAddress(Node *node) {
  ...
  if (!sym->codegen().llvmValue && currentComponent) {
    ...
    if (compSym && compSym->members.count(identName)) {
      ...
      auto selfExpr =
          std::make_unique<SelfExpression>(selfToken, std::move(fields));
...
llvm::Value *IRGenerator::generateArraySubscriptAddress(Node *node) {
  ...
  if (!arrExpr)
    reportDevBug("Null subscript expression", node);

  auto baseSym = semantics.getSymbolFromMeta(arrExpr);
  auto identSym = semantics.getSymbolFromMeta(arrExpr->identifier.get());

  if (!baseSym)
    reportDevBug("Missing array subscript symbol info", arrExpr);
  if (!identSym)
    reportDevBug("Missing  subscript ident symbol info", arrExpr);
  ...
  for (size_t i = 0; i < arrExpr->index_exprs.size(); ++i) {
    ...
    if (i < dims.size() && dims[i] > 0) {
      ...
      if (limit) {
        ...
        limit =
            funcBuilder.CreateIntCast(limit, funcBuilder.getInt64Ty(), false);
      } else {
        logInternal("  -> !! FAILURE !!: generateExpression returned NULL for "
                    "dynamic dim.");
      }
    }
    // Bounds Check
    if (limit) {
      ...
      logInternal(
          "  -> Skipping bounds check (No limit found for this level).");
    }
    ...
    for (size_t j = i + 1; j < maxDims; ++j) {
      ...
      if (dimSize) {
        currentStride =
            funcBuilder.CreateMul(currentStride, dimSize, "stride_update");
      }
    }

    llvm::Value *scaledIdx =
        funcBuilder.CreateMul(idx, currentStride, "index_stride");
