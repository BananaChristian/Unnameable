...
void IRGenerator::generateAssignmentStatement(Node *node) {
  ...
  if (assignMeta->storage().isHeap) {
    ...
    if (valSym && valSym->type().isAddress) {
      valueToStore = funcBuilder.CreateLoad(
          getLLVMType(assignMeta->type().type), initValue, "loaded_val");
  ...
  inhibitCleanUp = false;
  logInternal("[DEBUG] inhibitCleanUp before emitCleanup: " +
              std::to_string(inhibitCleanUp));
  emitCleanup(assignStmt);
...
void IRGenerator::generateFieldAssignmentStatement(Node *node) {
  ...
  bool isHuge = fieldSym->type().type.isBase() &&
                (fieldSym->type().type.kind == DataType::COMPONENT ||
  ...
  if (fieldSym->type().type.isArray() || isHuge) {
    ...
    llvm::Type *elemLLVMTy =
        getLLVMType(fieldSym->type().type.isArray()
                        ? semantics.getArrayElementType(fieldSym->type().type)
                        : fieldSym->type().type);
    llvm::Align align = layout->getABITypeAlign(elemLLVMTy);
...
void IRGenerator::registerGeneratorFunctions() {
  ...
  generatorFunctionsMap[typeid(ASMStatement)] =
      &IRGenerator::generateASMStatement;
}
...
void IRGenerator::registerExpressionGeneratorFunctions() {
  ...
  expressionGeneratorsMap[typeid(FStringLiteral)] =
      &IRGenerator::generateFStringLiteral;
...
bool IRGenerator::isDynamicArrayLiteral(ArrayLiteral *literal) {
  for (const auto &element : literal->array) {
    if (auto nested = dynamic_cast<ArrayLiteral *>(element.get())) {
      if (isDynamicArrayLiteral(nested))
        return true;
...
void IRGenerator::emitDynamicInitialization(ArrayLiteral *arrLit,
                                            llvm::Value *ptr,
  ...
  for (auto &element : arrLit->array) {
    if (auto nested = dynamic_cast<ArrayLiteral *>(element.get())) {
      ...
      auto *slotPtr =
          funcBuilder.CreateGEP(baseTy, ptr, funcBuilder.getInt64(index++));

      // The Baton sees this Store and says: "Inhibit dealloc for the source
      // ID!"
...
void IRGenerator::traceRuntime(llvm::Value *val, ResolvedType type) {
  ...
  // auto ptrTy = llvm::PointerType::getUnqual(context); // Generic Opaque
  // Pointer
...
void IRGenerator::executePhysicalFree(const std::shared_ptr<SymbolInfo> &sym) {
  ...
  logInternal("  [EMITTING-CALL] @ " + deallocatorName + " for " +
              sym->codegen().ID);

  llvm::Value *castPtr = funcBuilder.CreatePointerCast(
      sym->codegen().llvmValue,
      deallocFunc->getFunctionType()->getParamType(0));
...
void IRGenerator::emitDeclarationClean(Node *contextNode) {
  ...
  if (baton->ptrCount == 0 && baton->refCount == 0) {
    ...
    baton->isResponsible = false;
    logInternal("  [DECL-CLEAN] Responsibility revoked for: " +
                sym->codegen().ID);
  } else {
...
void IRGenerator::emitCleanup(Node *contextNode) {
  logInternal("[DEBUG] inhibitCleanUp during emitCleanup: " +
              std::to_string(inhibitCleanUp) +
  ...
  if (dynamic_cast<Identifier *>(contextNode) != contextNode)
    logInternal("[NORMAL-CLEAN] Composite node: " + contextNode->toString() +
                " detected");
...
llvm::Value *IRGenerator::stringizeValue(llvm::Value *val,
  ...
  if (type.isPointer() || type.kind == DataType::OPAQUE) {
    llvm::Value *addrInt =
        funcBuilder.CreatePtrToInt(val, llvm::Type::getInt64Ty(context));
    llvm::Value *buf =
        funcBuilder.CreateAlloca(i8Ty, 0, funcBuilder.getInt64(32), "ptr_buf");
  ...
  if (type.isBase()) {
    ...
    case DataType::USIZE: {
      ...
      bool isSigned =
          (type.kind == DataType::I8 || type.kind == DataType::I16 ||
      ...
      llvm::Value *buf = funcBuilder.CreateAlloca(
          i8Ty, 0, funcBuilder.getInt64(64), "int_buf");
    ...
    case DataType::F64: {
      ...
      llvm::Value *buf = funcBuilder.CreateAlloca(
          i8Ty, 0, funcBuilder.getInt64(64), "float_buf");
    ...
    case DataType::CHAR32: {
      llvm::Value *buf = funcBuilder.CreateAlloca(
          i8Ty, 0, funcBuilder.getInt64(2), "char_buf");
...
llvm::Function *IRGenerator::getOrDeclareStrlen() {
  ...
  return llvm::Function::Create(fty, llvm::Function::ExternalLinkage, "strlen",
                                *module);
}

llvm::Function *IRGenerator::getOrDeclareUnnitoa() {
  if (auto *f = module->getFunction("unnitoa"))
    return f;
  ...
  return llvm::Function::Create(fty, llvm::Function::ExternalLinkage, "unnitoa",
                                *module);
}

llvm::Function *IRGenerator::getOrDeclareUnnftoa() {
  if (auto *f = module->getFunction("unnftoa"))
    return f;
  auto f64Ty = llvm::Type::getDoubleTy(context);
  auto ptrTy = llvm::PointerType::getUnqual(context);
...
llvm::Function *IRGenerator::getOrDeclareUnniptoa() {
  if (auto *func = module->getFunction("unniptoa"))
    return func;
  auto i64Ty = llvm::Type::getInt64Ty(context);
  auto ptrTy = llvm::PointerType::getUnqual(context);
...
llvm::Function *IRGenerator::getOrDeclareStrcat() {
  if (auto *f = module->getFunction("unn_strcat"))
    return f;
  auto ptrTy = llvm::PointerType::getUnqual(context);
...
llvm::Function *IRGenerator::getOrDeclareWrite() {
  ...
  llvm::Function *writeFunc = llvm::Function::Create(
      funcTy, llvm::Function::ExternalLinkage, "write", *module);
