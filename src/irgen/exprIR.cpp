...
llvm::Value *IRGenerator::generateStringLiteral(Node *node) {
    ...
    if (!strLit) reportDevBug("Invalid string literal", node);
    ...
    llvm::Constant *strPtr =
        llvm::ConstantExpr::getInBoundsGetElementPtr(strConst->getType(), globalStr, indices);
...
llvm::Value *IRGenerator::generateFStringLiteral(Node *node) {
    ...
    if (!fStr) reportDevBug("Invalid f-string literal", node);
    ...
    for (const auto &seg : fStr->segments) {
        ...
        if (seg.string_part) {
            ...
            funcBuilder.CreateMemCpy(writePos, llvm::MaybeAlign(1), litPtr, llvm::MaybeAlign(1),
                                     len);
        ...
        for (auto &valExpr : seg.values) {
            ...
            if (!val) reportDevBug("Failed to generate value ", valExpr.get());

            auto sym = semantics.getSymbolFromMeta(valExpr.get());
            if (!sym) reportDevBug("Failed to get value symbol info", valExpr.get());

            llvm::Value *strVal = stringizeValue(val, sym->type().type);
            llvm::Value *strLen = funcBuilder.CreateCall(getOrDeclareStrlen(), {strVal});

            llvm::Value *writePos = funcBuilder.CreateGEP(i8Ty, buffer, cursor);
            funcBuilder.CreateMemCpy(writePos, llvm::MaybeAlign(1), strVal, llvm::MaybeAlign(1),
                                     strLen);
...
llvm::Value *IRGenerator::generateChar8Literal(Node *node) {
    ...
    if (!charLit) reportDevBug("Invalid char literal", node);
...
llvm::Value *IRGenerator::generateChar16Literal(Node *node) {
    ...
    if (!char16Lit) reportDevBug("Invalid char 16 literal", node);
...
llvm::Value *IRGenerator::generateChar32Literal(Node *node) {
    ...
    if (!char32Lit) reportDevBug("Invalid char32 literal", node);
...
llvm::Value *IRGenerator::generateBooleanLiteral(Node *node) {
    ...
    if (!boolLit) reportDevBug("Invalid boolean type", node);
...
llvm::Value *IRGenerator::generateI8Literal(Node *node) {
    ...
    if (!i8Lit) reportDevBug("Invalid i8 literal", node);
...
llvm::Value *IRGenerator::generateU8Literal(Node *node) {
    ...
    if (!u8Lit) reportDevBug("Invalid u8 literal", node);
...
llvm::Value *IRGenerator::generateI16Literal(Node *node) {
    ...
    if (!i16Lit) reportDevBug("Invalid i16 literal", node);
...
llvm::Value *IRGenerator::generateU16Literal(Node *node) {
    ...
    if (!u16Lit) reportDevBug("Invalid u16 literal", node);
...
llvm::Value *IRGenerator::generateI32Literal(Node *node) {
    ...
    if (!i32Lit) reportDevBug("Invalid i32 literal", node);
...
llvm::Value *IRGenerator::generateU32Literal(Node *node) {
    ...
    if (!u32Lit) reportDevBug("Invalid u32 literal", node);
...
llvm::Value *IRGenerator::generateI64Literal(Node *node) {
    ...
    if (!i64Lit) reportDevBug("Invalid i64 literal", node);
...
llvm::Value *IRGenerator::generateU64Literal(Node *node) {
    ...
    if (!u64Lit) reportDevBug("Invalid u64 literal", node);
...
llvm::Value *IRGenerator::generateI128Literal(Node *node) {
    ...
    if (!i128Lit) reportDevBug("Invalid i128 literal", node);
...
llvm::Value *IRGenerator::generateU128Literal(Node *node) {
    ...
    if (!u128Lit) throw std::runtime_error("Invalid u128 literal");
...
llvm::Value *IRGenerator::generateINTLiteral(Node *node) {
    ...
    if (!intLit) reportDevBug("Invalid int literal", node);

    auto litSym = semantics.getSymbolFromMeta(intLit);
    if (!litSym) reportDevBug("Failed to get literal symbol info", intLit);

    bool isSigned = isSignedInteger(litSym->type().type.kind);
    uint32_t bitWidth = convertIntTypeToWidth(litSym->type().type.kind);

    return generateIntegerLiteral(intLit->int_token.TokenLiteral, bitWidth, isSigned);
}
...
llvm::Value *IRGenerator::generateF32Literal(Node *node) {
    ...
    if (!f32Lit) reportDevBug("Invalid f32 literal", node);
...
llvm::Value *IRGenerator::generateF64Literal(Node *node) {
    ...
    if (!f64Lit) reportDevBug("Invalid f64 literal", node);

    double value = std::stod(f64Lit->f64_token.TokenLiteral);  // Converting the double literal from
...
llvm::Value *IRGenerator::generateFloatLiteral(Node *node) {
    ...
    if (!fltLit) reportDevBug("Invalid generic float literal", node);

    auto sym = semantics.getSymbolFromMeta(fltLit);
    if (!sym) reportDevBug("Failed to get literal symbol info", fltLit);
...
llvm::Value *IRGenerator::generateIdentifierExpression(Node *node) {
    ...
    if (!sym) reportDevBug("Failed to get identifier symbol info", identExpr);
    ...
    if (!loadedType) {
        reportDevBug("Type mapper failed for scalar '" + identName + "'", identExpr);
    }

    // variableAddr should already be a pointer, load from it
    llvm::LoadInst *val = funcBuilder.CreateLoad(loadedType, address, identName + "_val");
    if (sym->storage().isVolatile) {
...
void IRGenerator::flattenArrayLiteral(ArrayLiteral *arrLit,
    ...
    for (auto &element : arrLit->array) {
        if (auto nested = dynamic_cast<ArrayLiteral *>(element.get())) {
            ...
            if (!constVal) throw std::runtime_error("Array literal element must be constant");

            if (!baseType) baseType = constVal->getType();
            flatElems.push_back(constVal);
        }
    }
}

llvm::Value *IRGenerator::generateStaticArray(ArrayLiteral *lit) {
    // Steamroll the literal into a flat vector of scalars
    std::vector<llvm::Constant *> flatElems;
    llvm::Type *baseType = nullptr;
    flattenArrayLiteral(lit, flatElems, baseType);

    if (flatElems.empty() || !baseType) {
        reportDevBug("Empty or invalid array literal", lit);
    }

    // Create a flat LLVM Array Type [TotalCount x BaseType]
    llvm::ArrayType *flatArrayTy = llvm::ArrayType::get(baseType, flatElems.size());
    llvm::Constant *constantData = llvm::ConstantArray::get(flatArrayTy, flatElems);

    //  Shove it into a Global Variable (The Data Segment)
    // This is like 'static const int hidden_data[] = {1, 2, 3, 4};' in C
    auto *globalData = new llvm::GlobalVariable(*module, flatArrayTy,
                                                true,  // Constant? Yes.
                                                llvm::GlobalValue::PrivateLinkage, constantData,
                                                "flat_array_literal");
...
llvm::Value *IRGenerator::generateArrayLiteral(Node *node) {
    ...
    if (!arrLit) reportDevBug("Invalid array literal", node);

    auto litSym = semantics.getSymbolFromMeta(arrLit);
    ...
    // Allocate runtime space (e.g., on stack for temp literals)
    llvm::Value *arrayPtr = funcBuilder.CreateAlloca(baseTy, funcBuilder.getInt64(totalElements));
...
llvm::Value *IRGenerator::generateNullLiteral(Node *node) {
    ...
    if (!nullLit) reportDevBug("Invalid null literal", node);

    auto nullSym = semantics.getSymbolFromMeta(nullLit);
    if (!nullSym) reportDevBug("Failed to get null symbol info", nullLit);
...
llvm::Value *IRGenerator::generateInfixExpression(Node *node) {
    ...
    if (!infix) reportDevBug("Invalid infix expression", node);

    llvm::Value *result = nullptr;
    if (infix->operat.type == TokenType::SCOPE_OPERATOR) return handleEnumAccess(infix);
    ...
    auto promoteInt = [&](llvm::Value *val, DataType fromType, DataType toType) -> llvm::Value * {
        ...
        if (fromBits == 0 || toBits == 0) return val;  // Not integer type
        if (fromBits == toBits) return val;            // Same bit width, no promotion needed

        bool fromSigned = isSignedInteger(fromType);
        if (toBits > fromBits) {
            if (fromSigned)
                return funcBuilder.CreateSExt(val, llvm::IntegerType::get(context, toBits),
                                              "sexttmp");
            else
                return funcBuilder.CreateZExt(val, llvm::IntegerType::get(context, toBits),
                                              "zexttmp");
        } else {
            // Truncation if needed (rare, probably invalid here)
            return funcBuilder.CreateTrunc(val, llvm::IntegerType::get(context, toBits),
                                           "trunctmp");
        }
    };
    ...
    if (infix->operat.type == TokenType::COALESCE) result = handleCoalesce(infix, leftVal);
    ...
    if (resultType.kind == DataType::F32) {
        if (isIntegerType(leftSym->type().type.kind))
            leftVal =
                funcBuilder.CreateSIToFP(leftVal, llvm::Type::getFloatTy(context), "inttof32");
        if (isIntegerType(rightSym->type().type.kind))
            rightVal =
                funcBuilder.CreateSIToFP(rightVal, llvm::Type::getFloatTy(context), "inttof32");
    } else if (resultType.kind == DataType::F64) {
        if (isIntegerType(leftSym->type().type.kind))
            leftVal =
                funcBuilder.CreateSIToFP(leftVal, llvm::Type::getDoubleTy(context), "inttof64");
        if (isIntegerType(rightSym->type().type.kind))
            rightVal =
                funcBuilder.CreateSIToFP(rightVal, llvm::Type::getDoubleTy(context), "inttof64");
    }
...
llvm::Value *IRGenerator::generatePrefixExpression(Node *node) {
    ...
    if (!prefix) reportDevBug("Invalid prefix expression", node);

    if (isGlobalScope)
        reportDevBug("Executable statements arent allowed in the global scope", prefix);

    llvm::Value *operand = generateExpression(prefix->operand.get());

    if (!operand) throw std::runtime_error("Failed to generate IR for prefix operand");

    const std::string &name = prefix->operand->expression.TokenLiteral;
    auto prefixSym = semantics.getSymbolFromMeta(prefix);
    if (!prefixSym) reportDevBug("Unknown variable '" + name + "' in prefix", prefix);
    ...
    switch (prefix->operat.type) {
        ...
        case TokenType::MINUS_MINUS: {
            ...
            if (!ident) throw std::runtime_error("Prefix ++/-- must be used on a variable");
            ...
            llvm::Value *loaded = funcBuilder.CreateLoad(varType, address, llvm::Twine("loadtmp"));
            ...
            if (prefix->operat.type == TokenType::PLUS_PLUS)
                updated = (resultType.kind == DataType::F32 || resultType.kind == DataType::F64)
...
llvm::Value *IRGenerator::generatePostfixExpression(Node *node) {
    ...
    if (!postfix) reportDevBug("Invalid postfix expression", node);

    auto identifier = dynamic_cast<Identifier *>(postfix->operand.get());
    if (!identifier) reportDevBug("Postfix operand must be a variable", identifier);
    ...
    llvm::Type *varType = getLLVMType(resultType);
    if (!varType)
        reportDevBug("Invalid type for variable: " + identifier->identifier.TokenLiteral, postfix);

    llvm::Value *originalValue = funcBuilder.CreateLoad(varType, address, llvm::Twine("loadtmp"));
    ...
    if (postfix->operator_token.type == TokenType::PLUS_PLUS)
        updatedValue = (resultType.kind == DataType::F32 || resultType.kind == DataType::F64)
        ...
        throw std::runtime_error(
            "Unsupported postfix operator: " + postfix->operator_token.TokenLiteral + " at line " +
...
llvm::Value *IRGenerator::generateUnwrapExpression(Node *node) {
    ...
    if (!unwrapExpr) reportDevBug("Invalid unwrap expression", node);

    if (isGlobalScope) reportDevBug("Cannot unwrap in global scope", unwrapExpr);
    ...
    llvm::BasicBlock *successBB = llvm::BasicBlock::Create(context, "unwrap.success", currentFunc);
    llvm::BasicBlock *panicBB = llvm::BasicBlock::Create(context, "unwrap.panic", currentFunc);
...
llvm::Value *IRGenerator::generateCastExpression(Node *node) {
    ...
    if (!castExpr) reportDevBug("Invalid cast expression", node);

    auto castSym = semantics.getSymbolFromMeta(castExpr);
    if (!castSym) reportDevBug("Failed to get cast symbol info", castExpr);

    llvm::Value *sourceVal = generateExpression(castExpr->expr.get());
    if (!sourceVal) throw std::runtime_error("Failed to generate source value");

    llvm::Type *srcType = sourceVal->getType();
    llvm::Type *dstType = getLLVMType(castSym->type().type);

    if (srcType == dstType) return sourceVal;
...
llvm::Value *IRGenerator::generateBitcastExpression(Node *node) {
    ...
    if (!bitcastExpr) reportDevBug("Invalid bitcast expression", node);

    auto bitcastSym = semantics.getSymbolFromMeta(bitcastExpr);
    if (!bitcastSym) reportDevBug("Failed to get bitcast symbol info", bitcastExpr);

    // Generate the source value (the bits we are reinterpreting)
    llvm::Value *sourceVal = generateExpression(bitcastExpr->expr.get());
    if (!sourceVal) reportDevBug("Failed to generate source value", bitcastExpr->expr.get());
    ...
    if (srcType->isIntegerTy() && dstType->isPointerTy()) {
        ...
        if (srcType->getIntegerBitWidth() < ptrWidth) {
            sourceVal =
                funcBuilder.CreateSExt(sourceVal, funcBuilder.getIntNTy(ptrWidth), "bitcast_sext");
        }
...
llvm::Value *IRGenerator::generateSizeOfExpression(Node *node) {
    ...
    if (!sizeOf) reportDevBug("Invalid sizeof expression", node);
...
llvm::Value *IRGenerator::generateSelfExpression(Node *node) {
    ...
    if (!selfExpr) reportDevBug("Invalid self expression", node);
...
llvm::Value *IRGenerator::generateDereferenceExpression(Node *node) {
    ...
    for (int i = 0; i < derefCount; i++) {
        ...
        auto *hopLoad = funcBuilder.CreateLoad(loadTy, addr, isLastHop ? "deref_val" : "ptr_hop");
        if (meta && meta->storage().isVolatile) {
...
llvm::Value *IRGenerator::generateAddressExpression(Node *node) {
    ...
    if (!sym) reportDevBug("Failed to find address symbol info", addrExpr);
