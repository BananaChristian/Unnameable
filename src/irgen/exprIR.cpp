...
llvm::Value *IRGenerator::generateFStringLiteral(Node *node) {
  ...
  llvm::Value *buffer =
      funcBuilder.CreateAlloca(i8Ty, 0, requiredSize, "fstr_tmp");
  ...
  for (const auto &seg : fStr->segments) {
    ...
    if (seg.string_part) {
      ...
      funcBuilder.CreateMemCpy(writePos, llvm::MaybeAlign(1), litPtr,
                               llvm::MaybeAlign(1), len);
      cursor =
          funcBuilder.CreateAdd(cursor, llvm::ConstantInt::get(i64Ty, len));
    }
    ...
    for (auto &valExpr : seg.values) {
      ...
      llvm::Value *writePos = funcBuilder.CreateGEP(i8Ty, buffer, cursor);
      funcBuilder.CreateMemCpy(writePos, llvm::MaybeAlign(1), strVal,
                               llvm::MaybeAlign(1), strLen);
...
llvm::Value *IRGenerator::generateF64Literal(Node *node) {
  ...
  double value = std::stod(
      f64Lit->f64_token.TokenLiteral); // Converting the double literal from
...
llvm::Value *IRGenerator::generateIdentifierExpression(Node *node) {
  ...
  if (sym->type().isFnPtr && sym->isFunction) {
    ...
    if (!fn)
      reportDevBug("Function '" + identName + "' not found in module",
                   identExpr);
    inhibitCleanUp = wasInhibited;
    return fn;
  }
  ...
  if (isGlobalScope) {
    auto *globalVar =
        llvm::dyn_cast<llvm::GlobalVariable>(sym->codegen().llvmValue);
    if (globalVar && globalVar->hasInitializer()) {
      return globalVar->getInitializer();
    }
    reportDevBug("Cannot load global '" + identName +
                     "' in constant initializer",
                 identExpr);
  }
...
llvm::Value *IRGenerator::generateArrayLiteral(Node *node) {
  ...
  if (!arrLit)
    reportDevBug("Invalid array literal", node);

  auto litSym = semantics.getSymbolFromMeta(arrLit);

  if (!isDynamicArrayLiteral(arrLit))
    return generateStaticArray(arrLit);
  ...
  llvm::Value *arrayPtr =
      funcBuilder.CreateAlloca(baseTy, funcBuilder.getInt64(totalElements));
...
llvm::Value *IRGenerator::generatePrefixExpression(Node *node) {
  ...
  if (isGlobalScope)
    reportDevBug("Executable statements arent allowed in the global scope",
                 prefix);
...
llvm::Value *IRGenerator::generateCastExpression(Node *node) {
  ...
  throw std::runtime_error("Unhandled cast from " +
                           castSym->type().type.resolvedName);
}
...
llvm::Value *IRGenerator::generateSelfExpression(Node *node) {
  ...
  for (size_t i = 0; i < selfExpr->fields.size(); ++i) {
    ...
    std::string fieldName =
        semantics.extractIdentifierName(selfExpr->fields[i].get());
    ...
    if (llvmIt == llvmCustomTypes.end())
      reportDevBug("LLVM struct missing for type '" + currentTypeName + "'",
                   selfExpr);
  ...
  bool lastFieldIsCall =
      dynamic_cast<CallExpression *>(selfExpr->fields.back().get()) != nullptr;
  ...
  if (lastFieldIsCall) {
    ...
    if (!targetFunc)
      reportDevBug("Self method '" + methodName + "' not found in module",
                   selfExpr);

    // Collect any call arguments from the CallExpression node
    auto callNode =
        dynamic_cast<CallExpression *>(selfExpr->fields.back().get());
    ...
    std::vector<llvm::Value *> userArgs =
        prepareArguments(targetFunc, callNode->parameters, 1, coercion);
