...
llvm::Value *IRGenerator::generateMethodCallExpression(Node *node) {
  ...
  if (isGlobalScope)
    reportDevBug("Cannot use calls in global scope", metCall);

  auto metSym = semantics.getSymbolFromMeta(metCall);

  // Check if there are semantic errors
  if (!metSym)
    reportDevBug("Semantic error detected on component method call", metCall);
  ...
  if (sealIt != semantics.sealTable.end()) {
    ...
    if (sealFnIt == sealFnMap.end()) {
      reportDevBug("Function '" + callName + "' does not exist in seal '" +
                       instanceName + "'",
                   metCall);
    }
    ...
    if (!objectPtr)
      reportDevBug("Failed to get llvm value for instance", instance);

    auto metCallSym = semantics.getSymbolFromMeta(metCall);
    if (!metCallSym)
      reportDevBug("Failed to get method call symbolInfo", metCall->call.get());
    ...
    std::vector<llvm::Value *> userArgs =
        prepareArguments(targetFunc, call->parameters, 1, coercion);
    ...
    metCallSym->codegen().llvmType = result->getType();
    metCallSym->codegen().llvmValue = result;
