...
llvm::Value *IRGenerator::generateScalarStorage(VariableDeclaration *decl,
  ...
  if (sym->storage().isHeap) {
    ...
    if (!initVal)
      initVal = llvm::Constant::getNullValue(getLLVMType(sym->type().type));
  ...
  // Nullable boxing
  logInternal("Is the type nullable: " +
              std::to_string(sym->type().type.isNull));
...
llvm::Value *IRGenerator::generateArrayStorage(VariableDeclaration *decl,
  ...
  llvm::Type *elemTy =
      getLLVMType(semantics.getArrayElementType(sym->type().type));
  ...
  if (type_modifier && !type_modifier->dimensions.empty()) {
    ...
    while (currentModifier) {
      ...
      currentModifier =
          dynamic_cast<TypeModifier *>(currentModifier->inner_modifier.get());
    }

    // Override with Layout info if constant (to ensure consistency)
    if (sym->codegen().componentSize > 0) {
      uint64_t totalElems =
          sym->codegen().componentSize / layout->getTypeAllocSize(elemTy);
...
void IRGenerator::generateGlobalScalarLet(std::shared_ptr<SymbolInfo> sym,
  ...
  if (sym->type().isArray) {
          ...
          for (auto &elem : lit->array) {
            if (auto *subArr = dynamic_cast<ArrayLiteral *>(elem.get())) {
              ...
              if (auto *constElem = llvm::dyn_cast<llvm::Constant>(elemVal)) {
                ...
                reportDevBug("Array element must be constant in global scope",
                             elem.get());
              }
            }
          }
    ...
    auto *g = new llvm::GlobalVariable(
        *module, arrayTy, sym->storage().isConstant, linkage, init, letName);
  ...
  if (isRecordType(sym->type().type.resolvedName)) {
    ...
    if (value) {
      ...
      if (!init) {
        reportDevBug("Record initializer must be constant in global scope",
                     value);
      }
    } else if (sym->isExportable) {
      auto *g = new llvm::GlobalVariable(*module, getLLVMType(sym->type().type),
                                         sym->storage().isConstant,
                                         llvm::GlobalValue::ExternalLinkage,
                                         nullptr, // NO INITIALIZER
                                         letName);
      g->setAlignment(getCustomOrABIAlign(sym, g->getValueType()));
      sym->codegen().llvmValue = g;
      sym->codegen().llvmType = getLLVMType(sym->type().type);
      return;
    } else {
    ...
    auto *g = new llvm::GlobalVariable(*module, init->getType(),
                                       sym->storage().isConstant, linkage, init,
                                       letName);
    g->setAlignment(getCustomOrABIAlign(sym, g->getValueType()));
    sym->codegen().llvmValue = g;
  ...
  auto *g = new llvm::GlobalVariable(
      *module, varType, sym->storage().isConstant, linkage, init, letName);
...
IRGenerator::allocateDynamicHeapStorage(std::shared_ptr<SymbolInfo> sym,
  ...
  size_t allocSize =
      sym->codegen().componentSize; // Get the size for the allocation
  llvm::Value *sizeArg =
...
llvm::Align IRGenerator::getCustomOrABIAlign(std::shared_ptr<SymbolInfo> sym,
                                             llvm::Type *llvmTy) {
  const std::string &typeName = sym->type().type.resolvedName;
