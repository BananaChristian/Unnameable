...
Layout::Layout(Semantics& sem, llvm::LLVMContext& ctx, ErrorHandler& handler, bool verbose)
...
void Layout::calculatorDriver(Node* node) {
    ...
    if (it == calculatorFnsMap.end()) {
        logInternal("No calculator found for node '" + node->toString() + "' , Ignoring ...");
        return;
    }
...
void Layout::registerComponentCalculatorFns() {
    calculatorFnsMap[typeid(VariableDeclaration)] = &Layout::calculateVariableDeclarationSize;
...
void Layout::calculateVariableDeclarationSize(Node* node) {
    ...
    if (!sym) reportDevBug("Failed to find declaration '" + name + "' symbol info");
    ...
    if (sym->type().isArray) {
        ...
        if (declaration->initializer) {
            if (dynamic_cast<ArrayLiteral*>(initializer))
                ...
                if (!initSym) reportDevBug("Could not find initializer symbol info");
        ...
        if (!type_modifier->dimensions.empty() && isConstantLength) {
            if (declaration->initializer && totalElements != totalLiteralElements) {
                logLayoutError(
                    "Size mismatch between expected size '" + std::to_string(totalElements) +
                        "' elements and declared size '" + std::to_string(totalLiteralElements) +
        ...
        logInternal("Total Array Byte size :" + std::to_string(totalByteSize) + " bytes");
...
void Layout::calculateSwitchStatement(Node* node) {
    ...
    for (const auto& clause : switchStmt->case_clauses) calculatorDriver(clause.get());

    for (const auto& defs : switchStmt->default_statements) calculatorDriver(defs.get());
}
...
void Layout::calculateCaseClause(Node* node) {
    ...
    for (const auto& stmt : caseClause->body) calculatorDriver(stmt.get());
}
...
void Layout::calculateComponentStatement(Node* node) {
    ...
    if (!structTy->isOpaque()) return;  // already defined
...
llvm::Type* Layout::getLLVMType(const ResolvedType& type) {
    ...
    if (type.isArray()) {
        if (!type.innerType) throw std::runtime_error("Array type has no inner element type");
    ...
    switch (type.kind) {
        ...
        case DataType::ENUM: {
            ...
            if (enumIt == semantics.customTypesTable.end())
                throw std::runtime_error("Layout requested for unknown enum '" + type.resolvedName +
                                         "'");
        ...
        case DataType::UNKNOWN:
            throw std::runtime_error("Unsupported or unknown data type in getLLVMType: " +
                                     type.resolvedName);
    }
...
void Layout::declareAllCustomTypes() {
    ...
    for (const auto& [name, info] : semantics.customTypesTable) {
        ...
        if (typeIt == typeMap.end()) typeMap[name] = llvm::StructType::create(context, name);
    }
}
...
void Layout::registerImportedTypes() {
    ...
    for (const auto& compTypesPair : semantics.ImportedComponentTable) {
        ...
        for (const auto& memberPair : members) {
            ...
            if (memInfo->isFunction) continue;
