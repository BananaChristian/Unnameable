...
void Semantics::enforceDeclarationRules(VariableDeclaration* declaration,
    ...
    auto type_modifier = dynamic_cast<TypeModifier*>(declaration->modified_type.get());
    if (type_modifier) {
    ...
    if (resolveSymbolInfo(declName)) {
        std::string errorMsg = "Variable '" + declName + "' cannot be redeclared in the same scope";
    ...
    if (!initializer && isConstant) {
        ...
        logSemanticErrors("Constant variable '" + declName + "' must be initialized", declaration);
    }
    ...
    if (isConstant && !isConstLiteral(initializer)) {
        ...
        logSemanticErrors("Constant '" + declName +
    ...
    if (isConstant && isNullable) {
        ...
        logSemanticErrors("Constant '" + declName + "' cannot be nullable", declaration);
    }

    if (isPersist && isConstant) {
        errorHandler.addHint("'persist' delays baton-system frees on heap variables")
            .addHint(
                "Constants are compile-time values — the baton system never "
                "touches them")
            .addHint("Remove 'persist', constants manage themselves");
        logSemanticErrors("Constant '" + declName + "' cannot be marked 'persist'", declaration);
        return;
    }
    ...
    if (isVolatile && isConstant) {
        errorHandler
            .addHint(
                "'const' means compile-time fixed, 'volatile' means hardware "
                "can change it")
            .addHint("These are contradictory, pick one")
            .addHint("For read-only hardware registers, use 'volatile' alone");
        logSemanticErrors("Variable '" + declName + "' cannot be both 'volatile' and 'const'",
                          declaration);
        return;
    }

    if (isHeap && isConstant) {
        errorHandler.addHint("Constants are compile-time values, heap is runtime dynamic")
            .addHint("Remove 'heap' and let the compiler handle storage")
            .addHint("If you need a large constant, consider a global const instead");
        logSemanticErrors("Constant '" + declName + "' cannot be heap allocated", declaration);
        return;
    }
    ...
    if (isRestrict && isRef) {
        errorHandler.addHint("'restrict' is a pointer-only qualifier that indicates no aliasing")
        ...
        logSemanticErrors("Cannot apply 'restrict' to reference variable '" + declName + "'",
                          declaration);
        return;
    }
    ...
    if (isRestrict && !isPointer) {
        errorHandler.addHint("'restrict' only applies to pointers, it signals no aliasing")
            .addHint("Remove 'restrict' for non-pointer declarations")
            .addHint("Example of valid usage: restrict ptr i32 p -> addr x");
        logSemanticErrors("'restrict' cannot be applied to non-pointer variable '" + declName + "'",
                          declaration);
        return;
    }
    ...
    if (!insideRecord || !insideComponent) {
        if (isRef && !initializer) {
            ...
            logSemanticErrors("Reference variable '" + declName + "' must be initialized",
                              declaration);
        }
    }

    if (isRef && isNullable) {
        logSemanticErrors("Reference variable '" + declName + "' cannot be nullable", declaration);
        return;
    }
    ...
    if (isPersist && isRef) {
        ...
        logSemanticErrors("Reference '" + declName + "' cannot be marked 'persist'", declaration);
        return;
    }

    if (isVolatile && isRef) {
        errorHandler.addHint("'volatile' tells the compiler to always read fresh from memory")
            .addHint(
                "References are aliases, apply 'volatile' to the target "
                "variable instead")
            .addHint("Example: declare the heap target as volatile, then reference it");
        logSemanticErrors("Reference '" + declName + "' cannot be marked 'volatile'", declaration);
        return;
    }
    ...
    if (isHeap && isRef) {
        ...
        logSemanticErrors("Reference '" + declName + "' cannot be heap allocated", declaration);
        return;
    }
    ...
    if (isArray && !initializer) {
        if (type_modifier && type_modifier->dimensions.empty()) {
            ...
            logSemanticErrors("Array '" + declName + "' missing dimensions and initializer",
                              declaration);
            return;
        }
    }
...
void Semantics::handleNullInitializers(VariableDeclaration* declaration,
    ...
    if (!declInfo->type().isNullable) {
        ...
        logSemanticErrors("Cannot assign null to non-nullable variable '" + declName + "'",
                          declaration);
        return;
    }
    ...
    if (declInfo->type().isArray) {
        if (type_modifier->dimensions.empty()) {
            ...
            logSemanticErrors("Cannot assign null to array without dimensions", declaration);
        }
        return;
    }

    if (declInfo->type().isRef) {
        logSemanticErrors("Cannot assign null to a reference variable declaration '" + declName +
...
void Semantics::walkVariableDeclaration(Node* node) {
    ...
    if (declaration->allocator) {
        ...
        if (allocIt == allocatorMap.end()) {
            logSemanticErrors("Unknown allocator type '" + allocatorName + "'", allocator);
            return;
        }
    ...
    if (initializer) {
        ...
        if (!initSym) {
            ...
            logSemanticErrors("Variable '" + initName + "' is undefined", initializer);
            return;
        }
        ...
        if (declaredType.kind == DataType::OPAQUE && !declInfo->type().isPointer) {
            ...
            logSemanticErrors("'opaque' only applies to pointer declarations", declaration);
            return;
        }
        ...
        if (declInfo->type().isRef) {
            if (initSym->type().isNullable) {
                ...
                logSemanticErrors("Reference cannot reference nullable value", declaration);
                return;
            }
            ...
            if (!initSym->storage().isHeap && !initSym->storage().isGlobal) {
                ...
                logSemanticErrors("Cannot create reference to local variable '" + initName + "'",
                                  declaration);
                return;
            }
