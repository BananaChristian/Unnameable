...
void Semantics::walkInfixExpression(Node *node) {
    ...
    if (infixExpr->operat.type == TokenType::FULLSTOP ||
        ...
        // Resolve member
        auto memberInfo = resultOfScopeOrDot(infixExpr->operat.type, lhsType,
        ...
        logInternal("Infix Type for member access :" + memberInfo->type().type.resolvedName);
    ...
    if (infixType.kind == DataType::UNKNOWN) {
        logSemanticErrors("Infix cannot be of unknown type", infixExpr->left_operand.get());
    }
    ...
    if (postfixExpr->operator_token.type == TokenType::PLUS_PLUS ||
        ...
        if (auto ident = dynamic_cast<Identifier *>(postfixOperand)) {
            ...
            if (symbol->storage().isMutable == false) {
                logSemanticErrors("Cannot apply '" + postfixExpr->operator_token.TokenLiteral +
                                      "' to immutable variable '" + ident->expression.TokenLiteral +
                                      "'",
                                  postfixExpr);
                return;
            ...
            logSemanticErrors("Postfix operator '" + postfixExpr->operator_token.TokenLiteral +
                                  "' can only be applied to identifiers",
                              postfixExpr);
            return;
        }
...
void Semantics::walkUnwrapExpression(Node *node) {
    ...
    if (!exprSym) {
        logSemanticErrors("Unwrapped unknown identifier '" + exprName + "'", unwrapExpr);
        return;
    }
    ...
    if (!exprType.isNull) {
        logSemanticErrors("Cannot unwrap a concrete type '" + exprType.resolvedName + "'",
                          unwrapExpr);
        hasError = true;
    }

    if (exprType.kind == DataType::VOID || exprType.kind == DataType::UNKNOWN) {
        logSemanticErrors("Cannot unwrap type '" + exprType.resolvedName + "'", unwrapExpr);
        hasError = true;
    }
...
void Semantics::walkCastExpression(Node *node) {
    ...
    if (!destinationType.isBase()) {
        ...
        logSemanticErrors(
            "Cast destination '" + destinationType.resolvedName + "' must be a scalar type",
            destNode);
    }
    ...
    if (destinationType.isNull) {
        ...
        logSemanticErrors("Cannot cast to nullable type '" + destinationType.resolvedName + "'",
                          destNode);
    }

    auto isCastable = [](const ResolvedType &t) -> bool {
        if (!t.isBase()) return false;  // no pointers, refs, arrays
        switch (t.base().kind) {
            case DataType::I8:
            case DataType::U8:
            case DataType::I16:
    ...
    if (!isCastable(destinationType)) {
        ...
        logSemanticErrors("Invalid cast destination type '" + destinationType.resolvedName + "'",
                          destNode);
    }
    ...
    if (sourceType.isNull) {
        errorHandler.addHint("Unwrap the nullable before casting")
            .addHint("Use '?''?' or 'unwrap' to get the non-nullable value first");
        logSemanticErrors("Cannot cast from nullable type '" + sourceType.resolvedName + "'",
                          sourceExpr);
    }

    if (sourceType.isPointer()) {
        errorHandler.addHint("Pointers cannot be cast — use 'bitcast' instead")
            .addHint("Example: bitcast<usize>(myPtr) to get the address as integer");
        logSemanticErrors("Cannot cast from pointer type '" + sourceType.resolvedName + "'",
                          sourceExpr);
    }

    if (sourceType.isRef()) {
        errorHandler.addHint("References are aliases, cast the target variable directly")
            .addHint("Example: cast<i32>(targetVar) instead of casting the reference");
        logSemanticErrors("Cannot cast from reference type '" + sourceType.resolvedName + "'",
                          sourceExpr);
    }

    if (sourceType.isArray()) {
        errorHandler.addHint("Arrays cannot be cast, cast individual elements instead")
            .addHint("Example: cast<i32>(myArr[0])");
        logSemanticErrors("Cannot cast from array type '" + sourceType.resolvedName + "'",
                          sourceExpr);
    }

    if (!isCastable(sourceType)) {
        errorHandler.addHint("Valid cast sources: integers, floats, bool")
            .addHint("Custom types cannot be cast, use a conversion method instead");
        logSemanticErrors("Invalid cast source type '" + sourceType.resolvedName + "'", sourceExpr);
    }
...
void Semantics::walkBitcastExpression(Node *node) {
    ...
    if (!sourceExpr) reportDevBug("Failed to get bitcast source", bitcastExpr);

    walker(sourceExpr);
    auto sourceSym = getSymbolFromMeta(sourceExpr);
    if (!sourceSym) reportDevBug("Could not get source symbol for bitcast", sourceExpr);
    ...
    if (destinationType.isNull) {
        ...
        logSemanticErrors("Cannot bitcast to nullable type '" + destinationType.resolvedName + "'",
                          destNode);
    }

    if (!isBitcastable(sourceType)) {
        errorHandler.addHint("Bitcastable types: pointers, integers, floats, opaque")
            .addHint("References and arrays cannot be bitcast")
            .addHint("Bare custom types cannot be bitcast, use ptr MyType instead");
        logSemanticErrors("Cannot bitcast from type '" + sourceType.resolvedName + "'", sourceExpr);
    }

    if (!isBitcastable(destinationType)) {
        errorHandler.addHint("Bitcastable destinations: pointers, integers, floats, opaque")
            .addHint("References and arrays cannot be bitcast targets")
            .addHint("Bare custom types cannot be bitcast, use ptr MyType instead");
        logSemanticErrors("Cannot bitcast to type '" + destinationType.resolvedName + "'",
                          destNode);
    }
...
void Semantics::walkArraySubscriptExpression(Node *node) {
    ...
    if (arrSymbol->hasError) return;

    walker(arrExpr->identifier.get());
    for (const auto &expr : arrExpr->index_exprs) walker(expr.get());
    ...
    if (!arrType.isArray() && !arrType.isPointer()) {
        errorHandler.addHint("Subscripting is only permmited on arrays and pointers");
        logSemanticErrors("Type '" + arrType.resolvedName + "' cannot be indexed ", arrExpr);
    }
    ...
    for (size_t i = 1; i < arrExpr->index_exprs.size(); ++i) {
        if (!elementType || !elementType->innerType) break;
...
void Semantics::walkIdentifierExpression(Node *node) {
    ...
    if (!symbolInfo) {
        logSemanticErrors(" Use of undeclared identifer '" + identName + "'", identExpr);
        return;
    }
        ...
        if (infix->operat.type != TokenType::FULLSTOP &&
            ...
            errorHandler
                .addHint(
                    ...
                    "of that expression")
                .addHint(
                    "The 'addr' operator can only work on an expression with a "
                    "permanent "
                    "location an arithetic operation of any sorts does not have "
                    "this");
            logSemanticErrors("Cannot take address of an arithmetic expression or comparison",
                              infix);
            return;
        }
    }
    ...
    if (call || metCall) {
        errorHandler
            .addHint(
                ...
                "of that expression")
            .addHint(
                "The 'addr' operator can only work on an expression "
                "with a permanent location, function calls are "
                "temporary in nature and have no location ");
        logSemanticErrors("Cannot get the address to a temporary value '" + addrName + "'",
                          innerExpr);
        return;
    }
...
void Semantics::walkMoveExpression(Node *node) {
    ...
    if (auto infix = dynamic_cast<InfixExpression *>(innerExpr)) {
        ...
        if (infix->operat.type != TokenType::FULLSTOP &&
            ...
            errorHandler
                .addHint(
                    "When you use the 'move' operator you want to transfer "
                    "ownership of memory from one variable to another")
                .addHint(
                    "The move operator can only work on an expression with a "
                    "permanent "
                    "location an arithetic operation of any sorts does not have "
                    "this");
            logSemanticErrors("Cannot move of an arithmetic expression or comparison", infix);
            return;
        }
    }
    ...
    if (call || metCall) {
        errorHandler
            .addHint(
                ...
                "ownership of memory from one variable to another")
            .addHint(
                "The move operator can only work on an expression "
                "with a permanent location, function calls are "
                "temoprary in nature and have no location ");
        logSemanticErrors("Cannot move a temporary value '" + exprName + "'", innerExpr);
        return;
    }
    ...
    if (!symbolInfo->storage().isHeap) {
        logSemanticErrors("Cannot move variable '" + exprName + "' as it is stored on the stack",
                          innerExpr);
    }

    if (symbolInfo->type().type.isPointer() || symbolInfo->type().type.isRef()) {
        logSemanticErrors("Cannot move variable '" + exprName + "' as it is of a type '" +
                              symbolInfo->type().type.resolvedName + "'",
                          innerExpr);
    }

    if (symbolInfo->storage().isInvalid) {
        logSemanticErrors("Variable '" + exprName + "' was already moved", innerExpr);
    }
...
void Semantics::walkDereferenceExpression(Node *node) {
    ...
    if (!derefSym) {
        ...
        logSemanticErrors("Use of undeclared identifier '" + derefName + "'", derefExpr);
        return;
    }
    ...
    if (!derefSym->type().type.isPointer()) {
        ...
        logSemanticErrors(
            "Cannot dereference non-pointer type '" + derefSym->type().type.resolvedName + "'",
            derefExpr);
        return;
    }
    ...
    if (derefSym->type().type.isNull) {
        ...
        logSemanticErrors("Cannot dereference nullable pointer '" + derefName + "'", derefExpr);
        return;
    }
    ...
    if (!derefSym->type().type.innerType) {
        ...
        logSemanticErrors("Cannot dereference untyped pointer '" + derefName + "'", derefExpr);
        return;
    }

    if (derefSym->type().type.innerType->base().kind == DataType::OPAQUE) {
        errorHandler.addHint("Opaque pointers have no concrete type to dereference into")
            .addHint("Use bitcast to convert to a typed pointer first")
            .addHint("Example: bitcast<ptr i32>(myOpaquePtr)");
        logSemanticErrors("Cannot dereference opaque pointer '" + derefName + "'", derefExpr);
        return;
    }
