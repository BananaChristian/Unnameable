#include "prestatic.hpp"
#include <string>

Prestatic::Prestatic(Semantics &sem, llvm::LLVMContext &ctx)
...
// Calculator driver
void Prestatic::calculatorDriver(Node *node)
{
    if (!node)
    {
...
void Prestatic::registerComponentCalculatorFns()
{
    calculatorFnsMap[typeid(LetStatement)] = &Prestatic::calculateLetStatementSize;
}

// Independent calculators
void Prestatic::calculateLetStatementSize(Node *node)
{
...
void Prestatic::logPrestaticError(const std::string &message, int line, int col)
{
    std::cerr << "[PRESTATIC ERROR] " << message << " on line :" << line << "and column: " << col << "\n";
}

llvm::Type *Prestatic::getLLVMType(ResolvedType type)
{
    switch (type.kind)
    {
