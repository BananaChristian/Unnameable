...
struct ResolvedType {
  ...
  bool isFnPtr() const { return modifier == Modifier::FN_PTR; }
  ...
  std::string toString() const {
    ...
    case Modifier::FN_PTR: {
      std::string result = "fn(";
      for (size_t i = 0; i < fnParamTypes.size(); i++) {
        result += fnParamTypes[i].toString();
        if (i < fnParamTypes.size() - 1)
          result += ", ";
      }
      result += "):";
      result += fnReturnType ? fnReturnType->toString() : "void";
      return result;
    }
    case Modifier::NONE:
...
struct TypeInfo {
  ...
  std::vector<Node *>
      dynSizePerDimensions; // If the size isnt known at compile time we store
