...
private:
  ...
  llvm::Type *createCoercedType(const std::vector<Classification> &classes,
                                uint64_t size);
  ...
  void emitDynamicInitialization(ArrayLiteral *arrLit, llvm::Value *ptr,
  ...
  std::vector<llvm::Value *>
  prepareFnPtrArguments(const std::vector<ResolvedType> &paramTypes,
                        const std::vector<std::unique_ptr<Expression>> &params);
