...
private:
  ...
  void simulateDeclFree(VariableDeclaration *declaration,
                        const std::string &contextID);
  void simulateFree(Node *contextNode, const std::string &contextID);
  void freeCore(LifeTime *baton, const std::shared_ptr<SymbolInfo> &contextSym,
                Node *holderNode);
  void interceptHeist(Node *contextNode);

  // Loggers and reporters
  void logAuditError(ErrorCode code, Node *contextNode,
