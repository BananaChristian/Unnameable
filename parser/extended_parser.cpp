...
std::unique_ptr<Expression> Parser::parseLambdaExpression(std::unique_ptr<Expression> left)
{
    // `left` should be a tuple or param list
    std::vector<std::unique_ptr<Expression>> parameters;

    // Convert left expression into parameters
    if (auto tuple = dynamic_cast<TupleExpression *>(left.get()))
    {
        for (auto &element : tuple->elements)
        {
            parameters.push_back(std::move(element));
        }
    }
    else
    {
        parameters.push_back(std::move(left));
    }

    // Expect =>
    if (currentToken().type != TokenType::ARROW)
    {
        logError("Expected => after lambda parameters.");
    }
    advance(); // consume =>

    std::vector<std::unique_ptr<Expression>> blockOrReturn;

    // Handle either block or single expression
    if (currentToken().type == TokenType::LBRACE)
    {
        advance(); // Consume '{'
        auto blockContent = parseBlockExpression();
        blockOrReturn.push_back(std::move(blockContent));
    }
    else
    {
        auto returnContent = parseExpression(Precedence::PREC_ASSIGNMENT);
        blockOrReturn.push_back(std::move(returnContent));
    }

    return std::make_unique<LambdaExpression>(
        currentToken(),
        std::move(parameters),
        std::move(blockOrReturn));
}
